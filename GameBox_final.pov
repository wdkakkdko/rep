...
#elseif (clock >= 36 & clock < 96) // 終了タイミング: 76 -> 96
  #declare Claw_Rot = 10;
#elseif(clock >= 96 & clock < 106) // 開始/終了: 76,86 -> 96,106
  #declare T = (clock - 96) / 10;
  #declare Claw_Rot = 10 + (20 * T);
#elseif (clock >= 106) // 開始タイミング: 86 -> 106
  #declare Claw_Rot = 30;
#end
...
#declare Crane_Game_Upper = union {
    ...
    #elseif (clock < 96) // 終了タイミング: 76 -> 96
        #declare Move_Progress = (clock-46) / (96-46); // 期間を延長
        #declare Current_X = -0.7 + (1.325) * Move_Progress; // 最終地点までの距離で計算
        object { Claw_Unit translate <Current_X, (Y_Height - (12*0.025) + (10*0.025)), 0> }
    #else
        object { Claw_Unit translate <0.625, (Y_Height - (12*0.025) + (10*0.025)), 0> } // 最終地点(37.5/60)で静止
    #end
}
...
union {
    ...
    // ### ロボットのアニメーション ###
        ...
        object {
            ...
            #elseif (clock < 90) // 終了タイミング: 70 -> 90
                // 2. 穴の上まで横移動
                #declare Move_Progress = (clock - 46) / (90 - 46); // 期間: 24 -> 44
                #declare Current_X = Claw_Base_X + (Hole_X - Claw_Base_X) * Move_Progress;
                translate <Current_X, Hold_Y, Claw_Base_Z>

            #elseif (clock < 100) // 開始/終了: 70,80 -> 90,100
                // 3. 穴の上で少し待機
                translate <Hole_X, Hold_Y, Claw_Base_Z>

            #else
                // 4. 落下
                #declare Drop_Start_Time = 100; // 開始タイミング: 80 -> 100
